...
def predict():
    try:
        ...
        if "file" in request.files:
            ...
            response = send_file(output, mimetype="text/csv", as_attachment=True, download_name="predictions.csv")
            response.headers["X-Graph"] = base64.b64encode(img.getvalue()).decode("utf-8")
            return response

        # Case 2: Single text prediction (JSON input)
...
def predict_sentiment(text):
    ...
    print("Vector sum:", vectorized.sum())
    print("Raw prediction output:", prediction)

    # Convert prediction to int if not already, threshold if needed (for probabilities)
    pred_value = int(prediction[0]) if hasattr(prediction, "__getitem__") else int(prediction)
